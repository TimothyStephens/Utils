...
DESCRIPTION = """
	...
	NOTE:
		- Modified for output of MMSeqs2. This output is exactly the same as blast but MMSeqs2 already splits UniProit headers.
...
def main():
	...
	for key, hits in blast_dict.items():
		...
		for hit in hits:
			...
			goa_out = goa_db.submit_command('SELECT go_id FROM GOterm_mappings WHERE id=?', (hit, ))
