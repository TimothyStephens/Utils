...
Output:
# out
seq1\tseq1start\tseq1end\tseq2\tseq2start\tseq2end\tclassification\tbad_positions\n

# position_info (if file name provided)
seq1\tpos_start\tpos_end\tok_read_counts\tbad_read_counts\n
...
def check_4_misassemblies_between_two_points(bam_file, regions_file, out_file, position_info_file, region_size, min_ok_reads):
	...
	out_file.write("seq1\tseq1start\tseq1end\tseq2\tseq2start\tseq2end\tclassification\tbad_positions\n")
	if position_info_file is not None:
		position_info_file.write("seq1\tpos_start\tpos_end\tok_read_counts\tbad_read_counts\n")
	...
	for line in regions_file:
		...
		if len(bad_positions) > 0:
			out_file.write('\t'.join([seqid, str(start), str(start), seqid, str(stop), str(stop), "Failed_min_ok_reads", ','.join([str(x) for x in bad_positions])])+'\n')
		else:
			out_file.write('\t'.join([seqid, str(start), str(start), seqid, str(stop), str(stop), "Passed_min_ok_reads", "NA"])+'\n')
