...
P="test_data/test.SL_from_mapped_reads"
cat test_data/seq_names.txt | ./mapped_reads_with_SL_sequences.py -b $B -o $P.txt --info $P.SL_read_info.txt -s CCGGCTTTTCTG 2> $P.debug.log

O="test_data/orig.SL_from_mapped_reads"
...
## Full debug info
cat test_data/seq_names.txt | ./mapped_reads_with_SL_sequences.py -b $B -o $P.txt --info $P.SL_read_info.txt -s CCGGCTTTTCTG --debug 2> $P.debug.log_full
